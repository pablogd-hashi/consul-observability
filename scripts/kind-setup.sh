...
# ── 2. Pre-pull fake-service image into kind 
...
# ── 5. Install Consul (without --wait so we can overlap observability setup) ─
...
# ── 6. Deploy observability stack in parallel while Consul finishes ─────────
# Start the observability namespace and copy the ACL token while Consul pods
# are still initialising — they only need Prometheus/OTel later, not now.
...
until kubectl get secret consul-bootstrap-acl-token -n "$CONSUL_NS" &>/dev/null; do sleep 2; done
...
# ── 7. Wait for Consul server, then apply config entries while obs boots ────
...
# ── 8.5. Register 'rates' as an external service in Consul catalog ──────────
...
kubectl exec consul-server-0 -n "$CONSUL_NS" -- sh -c \
...
# ── 9. Deploy ALL fake-service manifests in one shot ────────────────────────
...
kubectl apply -f "${REPO_ROOT}/kubernetes/services/fake-service/"

# ── 10. Wait for everything in parallel ─────────────────────────────────────
# All pods are now scheduled — wait for the whole fleet at once.
info "Waiting for observability stack..."
kubectl rollout status deployment -l 'app in (loki,otel-collector,prometheus,jaeger,grafana)' \
  -n "$OBS_NS" --timeout=180s \
  || warn "Some observability pods did not become ready — check: kubectl get pods -n ${OBS_NS}"
...
echo "    kubectl port-forward svc/prometheus 9091:9090 -n $OBS_NS &"
echo ""
echo "  Consul UI (after port-forward):"
...
echo ""
echo "  Prometheus (after port-forward):"
echo "    http://localhost:9091"
echo ""
echo "  Jaeger (after port-forward):"
